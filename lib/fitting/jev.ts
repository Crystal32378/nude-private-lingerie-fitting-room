...
/** Direct TypeSafe System One (https://docs.typesafe.ai/api). */
...
export interface EvaluateResponse {
  ...
  /** Set by the transport that produced this response. */
...
export function createGatewayTransport(getToken: () => Promise<string | null>): JevTransport {
  return async (body, signal) => {
    ...
    return { ...data, transport: "vercel-ai-gateway",
      upstreamRequestId: typeof data.providerMetadata?.gateway?.generationId === "string" ? data.providerMetadata.gateway.generationId : null };
  };
}

/** Direct TypeSafe System One transport (the brand's own TypeSafe account).
 *  Same egress guard; the Gateway-only providerOptions are dropped. TypeSafe
 *  documents that it does not train on user data. Its response is normalised to
 *  the internal shape: confidence is only inline there, so inline IS the canonical
 *  value and is copied into providerMetadata; usage keys become camelCase. */
export function createTypesafeTransport(getKey: () => Promise<string | null>): JevTransport {
  return async (body, signal) => {
    assertHobbySafe(body);
    const key = await getKey();
    if (!key) throw new GatewayAuthError("no typesafe credential");
    ...
    if (res.status === 401 || res.status === 403) throw new GatewayAuthError(`typesafe ${res.status}`);
    if (!res.ok) throw new Error(`typesafe ${res.status}`);
    const data = await res.json() as { model?: unknown; id?: unknown; answers?: Record<string, any>; usage?: { input_tokens?: number; output_tokens?: number } };
    const answers = data.answers && typeof data.answers === "object" && !Array.isArray(data.answers) ? data.answers : {};
    const confidence: Record<string, number> = {};
    for (const [id, answer] of Object.entries(answers)) if (typeof answer?.confidence === "number") confidence[id] = answer.confidence;
    return {
      model: typeof data.model === "string" ? data.model : "",
      answers,
      usage: data.usage && Number.isFinite(data.usage.input_tokens) && Number.isFinite(data.usage.output_tokens)
        ? { inputTokens: data.usage.input_tokens!, outputTokens: data.usage.output_tokens! } : undefined,
      providerMetadata: { typesafe: { confidence } },
      transport: "typesafe-system-one",
      upstreamRequestId: res.headers.get("x-typesafe-request-id") ?? (typeof data.id === "string" ? data.id : null),
    };
  };
}

/** A response is accepted only from the model its transport promises. */
function acceptedModel(response: EvaluateResponse): boolean {
  return response.transport === "typesafe-system-one" ? /^jev-[\w.-]+$/.test(response.model) : response.model === MODEL_ID;
}
...
export async function evaluateTradeoff(input: unknown, transport: JevTransport = gatewayTransport): Promise<TradeoffResult> {
  ...
  try {
    ...
    if (!response || !acceptedModel(response) || !response.answers || typeof response.answers !== "object"
    ...
    const typesafe = response.transport === "typesafe-system-one";
    const receipt = {
      receiptId: `jev_${randomUUID().replace(/-/g, "").slice(0, 12)}`,
      transport: typesafe ? "typesafe-system-one" as const : "vercel-ai-gateway" as const,
      returnedModel: response.model,
      upstreamRequestId: response.upstreamRequestId ?? null,
      resultHash: createHash("sha256").update(JSON.stringify(judgments.map(j => [j.targetId, j.choice, j.probabilities, j.confidence ?? null]))).digest("hex").slice(0, 16),
      latencyMs,
    };
    return { task, status: "complete", judgments,
      ...
      provenance: { ...receipt, requestedModelId: typesafe ? TYPESAFE_MODEL : MODEL_ID, generationId: typeof response.providerMetadata?.gateway?.generationId === "string"
